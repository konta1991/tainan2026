...
(function(){
  ...
  function renderHome(){
    ...
    $("#home").innerHTML = `
      ...
      ${(()=>{ if(!tripDay) return ""; const d = DAYS.find(x=>x.key===tripDay); const ni = nowIndex(d); const next = d.items.slice(Math.max(0,ni), Math.max(0,ni)+3); if(!next.length) return ""; return `<div class="card"><h2>いまと、この後</h2><div class="next-list">${next.map((it,i)=>`<div class="next-item"><b>${esc(it.t)}</b><div>${esc(it.title)}${i===0&&ni>=0?'<span class="now-badge">いま</span>':""}<div class="muted small">${esc(it.body).slice(0,60)}</div></div></div>`).join("")}</div></div>`; })()}
  ...
  function nowIndex(d){
    if (d.key !== tripDay) return -1;
    const now = TN.h*60 + TN.mi; let idx = -1;
    d.items.forEach((it,i) => { const m = minutesOf(it.t); if (m !== null && m <= now) idx = i; });
    return idx;
  }
  function renderPlan(){
    ...
    const items = d.items.map((it,i) => `
      <div class="tl-item ${i===ni?"now":""} ${ni>=0&&i<ni?"past":""}" id="tl-${i}"><div class="time">${esc(it.t)}</div>
    ...
    const gn = $("#gonow"); if (gn) gn.addEventListener("click", () => { const el = $("#tl-"+ni); if (el) window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 120, behavior:"smooth" }); });
  }
  ...
  // ---- SHOP & SIGHT ----
  function renderShop(){
    const shops = SPOTS.filter(s => s.cat==="shop"), sights = SPOTS.filter(s => s.cat==="sight"), exps = SPOTS.filter(s => s.cat==="exp");
    $("#shop").innerHTML = `
      <div class="section-title">お土産・買い物</div>
      ...
      <div class="spot-list">${shops.map(spotCard).join("")}</div>
      <div class="section-title">見る・歩く</div>
      ...
      <div class="spot-list">${sights.map(spotCard).join("")}</div>
      <div class="section-title">夜の提灯さんぽ（徒歩のみ・約3時間）</div>
      <div class="card"><div class="small muted" style="margin-bottom:6px">日没18:25。普済殿の「提灯トンネル」は旧正月限定＝8月は無い。神農街の赤提灯は通年点灯＝そちらが本命。</div>
        <table>${NIGHTROUTE.map(r=>`<tr><th style="width:56px">${esc(r.t)}</th><td>${esc(r.p)}</td></tr>`).join("")}</table>
        <table style="margin-top:8px"><tr><th>ベスト撮影時間</th><th>時間帯</th></tr>
